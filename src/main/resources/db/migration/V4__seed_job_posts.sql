...
WHERE u.email = 'hr@techbridge.vn'
  AND NOT EXISTS (
    ...
    WHERE jp.employer_id = e.user_id AND jp.title = 'Backend Developer Java Spring Boot'
);
...
WHERE u.email = 'hr@techbridge.vn'
  AND NOT EXISTS (
    SELECT 1 FROM job_posts jp
    WHERE jp.employer_id = e.user_id AND jp.title = 'Frontend Developer ReactJS'
);
...
WHERE u.email = 'recruitment@datamind.vn'
  AND NOT EXISTS (
    SELECT 1 FROM job_posts jp
    WHERE jp.employer_id = e.user_id AND jp.title = 'Data Analyst'
);
...
WHERE u.email = 'recruitment@datamind.vn'
  AND NOT EXISTS (
    SELECT 1 FROM job_posts jp
    WHERE jp.employer_id = e.user_id AND jp.title = 'Machine Learning Engineer'
);
...
WHERE u.email = 'jobs@brightmarketing.vn'
  AND NOT EXISTS (
    SELECT 1 FROM job_posts jp
    WHERE jp.employer_id = e.user_id AND jp.title = 'Digital Marketing Specialist'
);
...
WHERE u.email = 'jobs@brightmarketing.vn'
  AND NOT EXISTS (
    SELECT 1 FROM job_posts jp
    WHERE jp.employer_id = e.user_id AND jp.title = 'UI UX Designer'
);
...
WHERE u.email = 'jobs@brightmarketing.vn'
  AND NOT EXISTS (
    SELECT 1 FROM job_posts jp
    WHERE jp.employer_id = e.user_id AND jp.title = 'Business Development Executive'
);
...
WHERE u.email = 'hr@techbridge.vn'
  AND NOT EXISTS (
    SELECT 1 FROM job_posts jp
    WHERE jp.employer_id = e.user_id AND jp.title = 'Technical Recruiter'
);
